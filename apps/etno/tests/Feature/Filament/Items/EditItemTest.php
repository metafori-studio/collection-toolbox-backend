...
it('saves and overrides correctly for primitive fields', function (string $column, mixed $parentValue, mixed $overrideValue) {
    ...
    livewire(EditItem::class, [
        ...
        ->set("data.$column", $overrideValue instanceof \BackedEnum ? $overrideValue->value : (is_array($overrideValue) && isset($overrideValue[0]) && $overrideValue[0] instanceof \BackedEnum ? array_map(fn ($e) => $e->value, $overrideValue) : $overrideValue))
        ->call('save')
...
it('validates and saves document_overrides correctly', function () {
    ...
    livewire(EditItem::class, [
        ...
        ->set('data.document_overrides', ['invalid_override_field'])
        ->call('save')
    ...
    livewire(EditItem::class, [
        ...
        ->set('data.document_overrides', [Item::INHERITABLES[0]])
        ->call('save')
    ...
    livewire(EditItem::class, [
        ...
        ->set('data.document_overrides', null)
        ->call('save')
        ->assertHasNoFormErrors(['document_overrides']);
...
it('saves and overrides correctly for precision date sections', function (string $sectionName, array $parentValues, array $overrideValues) {
    ...
    $livewire = livewire(EditItem::class, [
        'parentRecord' => $document,
        'record' => $item->id,
    ]);

    foreach ($overrideValues as $k => $v) {
        $livewire->set("data.$k", $v);
    }
    $livewire->set('data.document_overrides', array_keys($overrideValues));
